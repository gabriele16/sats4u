...
class Sats2Trade(lc.CryptoData, lc.Candles):
    ...
    def update_technical_indicators(self, in_step = 0, last_step = -1):
    ...
    def trade_loop(self, quantity):
        ...
        while True:
            try:
                ...
                # Update technical indicators and get signals
                signals_df = self.update_technical_indicators(self.candles)
                ...
                current_returns = (initial_balance / self.get_account_balance("USDT")) - 1
                cumulative_returns = current_returns + 1

                # Append the new returns to the returns dataframe
                self.returns_df = self.returns_df.append(
                    {"Date": pd.Timestamp.now(), "Returns": current_returns, "Cumulative Returns": cumulative_returns},
                    ignore_index=True,
                )
