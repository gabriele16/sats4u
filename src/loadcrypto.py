...
class CryptoData:
    def __init__(self, asset_details, data_folder):
        ...
        self.tick_to_id_df = pd.DataFrame(
            np.array([list(self.tick_to_id.keys()), list(self.tick_to_id.values())]).T, columns=["Ticker", "Asset_ID"]
        )
        ...
        while date_0 != date_1:
            ...
            for i, symbol in enumerate(tickers):
                ...
                if len(data_df) == 0:
                    ...
                    data_df = pd.concat([data_df.drop(columns=["timestamp", "Date"]), data_df_temp], axis=1)
